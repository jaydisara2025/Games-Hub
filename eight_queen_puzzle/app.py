...
def handle_player_answer(player_name, current_solution):
    try:
        if not player_name.strip():
            st.error("Player name cannot be empty or just spaces.")
        elif len(player_name.strip()) < 3:
            st.error("Player name must be at least 3 characters long.")
            return
        elif not all(c.isalpha() or c in " -'" for c in player_name.strip()):
            st.error("Player name can only contain letters, spaces, hyphens, or apostrophes.")
            return
